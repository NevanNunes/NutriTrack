...
public class HistoryController {
    ...
    private void onExport() {
        ...
        fileChooser.setTitle("Save Meal History CSV");
        fileChooser.getExtensionFilters().add(new FileChooser.ExtensionFilter("CSV Files", "*.csv"));
        fileChooser.setInitialFileName("meal_history.csv");

        Window ownerWindow = mealsTable.getScene().getWindow();
        File file = fileChooser.showSaveDialog(ownerWindow);

        if (file != null) {
            try (PrintWriter writer = new PrintWriter(file)) {
                // Write CSV header
                writer.println("Date,Time,MealType,Food,Portion (g),Calories,Protein (g),Carbs (g),Fat (g),Notes");
                // Write meal data
                for (Meal meal : filteredData) {
                    writer.printf("%s,%s,%s,%s,%.2f,%.2f,%.2f,%.2f,%.2f,%s\n",
                            DATE_FORMATTER.format(meal.getLogDate()),
                            meal.getCreatedAt().format(DateTimeFormatter.ofPattern("HH:mm")),
                            meal.getMealType(),
                            meal.getMealName(),
                            meal.getPortionSize() != null ? meal.getPortionSize() : 0.0,
                            meal.getCalories() != null ? meal.getCalories() : 0.0,
                            meal.getProteinG() != null ? meal.getProteinG() : 0.0,
                            meal.getCarbsG() != null ? meal.getCarbsG() : 0.0,
                            meal.getFatG() != null ? meal.getFatG() : 0.0,
                            meal.getNotes() != null ? meal.getNotes().replace(",", ";") : ""); // Replace commas in notes to avoid CSV issues
                }
                showAlert(Alert.AlertType.INFORMATION, "Export Successful", "Meal history exported to " + file.getAbsolutePath());
            } catch (Exception e) {
                showError("Failed to export meal history: " + e.getMessage());
                e.printStackTrace();
            }
        } else {
            showAlert(Alert.AlertType.INFORMATION, "Export Cancelled", "Meal history export was cancelled.");
        }
    }
